export const productCategories = [
  { id: 0, label: "Select Category", value: "" },
  { id: 1, label: "Airpodes", value: "airpodes" },
  { id: 2, label: "Camera", value: "camera" },
  { id: 3, label: "Earphones", value: "earphones" },
  { id: 4, label: "Mobiles", value: "mobiles" },
  { id: 5, label: "Earphones", value: "earphones" },
  { id: 6, label: "Mouse", value: "mouse" },
  { id: 7, label: "Printers", value: "printers" },
  { id: 8, label: "Speakers", value: "speakers" },
  { id: 9, label: "Trimmers", value: "trimmers" },
  { id: 10, label: "Speakers", value: "speakers" },
  { id: 11, label: "Watches", value: "watches" },
  { id: 12, label: "TVs", value: "tvs" },
  { id: 13, label: "Refrigerator", value: "refrigerator" },
  { id: 14, label: "Laptops", value: "laptops" },
  { id: 15, label: "Processors", value: "processors" },
];
